...
SELECT 
    ...
    COALESCE((SELECT SUM(v.valor) FROM emp_venc ev JOIN vencimento v ON ev.cod_venc = v.cod_venc WHERE ev.matr = e.matr), 0) AS salario_bruto,
    COALESCE((SELECT SUM(dsc.valor) FROM emp_desc ed JOIN desconto dsc ON ed.cod_desc = dsc.cod_desc WHERE ed.matr = e.matr), 0) AS total_descontos,
    COALESCE((SELECT SUM(v.valor) FROM emp_venc ev JOIN vencimento v ON ev.cod_venc = v.cod_venc WHERE ev.matr = e.matr) - 
             COALESCE((SELECT SUM(dsc.valor) FROM emp_desc ed JOIN desconto dsc ON ed.cod_desc = dsc.cod_desc WHERE ed.matr = e.matr), 0), 0) AS salario_liquido
FROM empregado e
JOIN departamento d ON e.lotacao = d.cod_dep
LEFT JOIN divisao dv ON e.lotacao_div = dv.cod_divisao
GROUP BY d.nome, e.nome, e.matr
