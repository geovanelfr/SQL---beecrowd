...
SELECT
	result.nome,
    result.salario_liquido as salario
FROM(
	SELECT
        salario.lotacao_div,
        salario.nome,
    ...
    FROM (
        SELECT
            ...
            x.departamento,
            x.divisao,
            SUM(x.salario - x.desconto) as salario_liquido
        FROM (
            SELECT
                e.lotacao_div,
                e.nome,
                ...
                dv.nome AS divisao,
                MAX(COALESCE((SELECT SUM(v.valor) FROM emp_venc ev JOIN vencimento v ON ev.cod_venc = v.cod_venc WHERE ev.matr = e.matr), 0)) AS salario,
                MAX(COALESCE((SELECT SUM(dsc.valor) FROM emp_desc ed JOIN desconto dsc ON ed.cod_desc = dsc.cod_desc WHERE ed.matr = e.matr), 0)) AS desconto
            FROM
                departamento d
            JOIN
            ...
            GROUP BY
                e.lotacao_div, e.nome, d.nome, dv.nome
        ) x
        GROUP BY
            x.lotacao_div, x.nome, x.departamento, x.divisao
    ) salario
    JOIN (
        SELECT
            x.departamento,
            x.divisao,
            ROUND(AVG(x.salario - x.desconto), 2) AS media
        FROM (
            SELECT
                e.nome,
                d.nome AS departamento,
            ...
            LEFT JOIN
                empregado e ON dv.cod_divisao = e.lotacao_div
            GROUP BY
                e.nome, d.nome, dv.nome
        ) x
        GROUP BY
            x.departamento, x.divisao
    ) media ON salario.departamento = media.departamento AND salario.divisao = media.divisao

    WHERE 
        salario.salario_liquido > media.media
    AND
        salario.salario_liquido >= 8000.00
    ORDER BY 
        salario.lotacao_div
) result