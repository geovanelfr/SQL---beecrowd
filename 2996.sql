...
FROM
	packages p
JOIN 
    users us ON p.id_user_sender = us.id
JOIN 
    users ur ON p.id_user_receiver = ur.id
WHERE
	(p.color LIKE 'blue' OR p.year = 2015)
AND 
	us.address not LIKE 'Taiwan' 
AND 
	ur.address not LIKE 'Taiwan' 
ORDER BY year DESC	
